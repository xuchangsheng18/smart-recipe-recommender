...
class MilvusIndexConstructionModule:
    ...
    def __init__(self,
                 host: str = "localhost",
                 port: int = 19530,
                 uri: Optional[str] = None,  # 新增：支持直接传入本地文件路径或云端URI
                 token: Optional[str] = None,  # 新增：支持云端数据库的密码/Token
    ...
    def _setup_client(self):
        """初始化Milvus客户端"""
        try:
            # 优先使用 uri 方式连接 (支持 Milvus Lite 本地文件 或 Zilliz Cloud)
            if self.uri:
                client_params = {"uri": self.uri}
                if self.token:
                    client_params["token"] = self.token
                self.client = MilvusClient(**client_params)
                logger.info(f"已连接到Milvus (URI模式): {self.uri}")
            else:
                # 兼容旧版的 host:port 方式 (Docker模式)
                self.client = MilvusClient(
                    uri=f"http://{self.host}:{self.port}"
                )
                logger.info(f"已连接到Milvus服务器 (Host模式): {self.host}:{self.port}")
    ...
    def _setup_embeddings(self):
        """初始化本地嵌入模型（支持 GPU 加速）"""
        # 定义你的本地模型文件夹路径
        local_model_path = r"E:\bge-small-zh-v1.5"

        # 💡 核心修改：检测系统是否有可用的 NVIDIA GPU
        device = "cuda" if torch.cuda.is_available() else "cpu"

        logger.info(f"正在加载本地嵌入模型: {local_model_path}，计算设备: {device.upper()}")

        self.embeddings = HuggingFaceEmbeddings(
            model_name=local_model_path,
            # 💡 修改这里：将写死的 'cpu' 替换为动态检测的 device 变量
            model_kwargs={'device': device},
            encode_kwargs={'normalize_embeddings': True}
        )

        logger.info(f"本地嵌入模型加载完成 (当前运行在 {device.upper()} 模式)")
